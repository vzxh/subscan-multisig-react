...
function TreasuryProposal({
  ...
  const inner = (
    <>
      <Labelled label={t<string>('proposed by')}>
        <InputAddress defaultValue={proposer} isDisabled value={proposer} withLabel={false} />
      </Labelled>
      <Labelled label={t<string>('beneficiary')}>
        <InputAddress defaultValue={beneficiary} isDisabled value={beneficiary} withLabel={false} />
      </Labelled>
  ...
  if (asInset) {
    return (
      <Inset className={className} {...insetProps}>
        {inner}
      </Inset>
    );
  }
