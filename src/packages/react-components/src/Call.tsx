...
function Call({
  ...
  const [{ hash, params, signature, signatureType, values }, setExtracted] = useState<Extracted>({
    hash: null,
    params: [],
    signature: null,
    signatureType: null,
      ...
      <div className="ui--Extrinsic--toplevel">
        {signature && (
          <Static
            ...
            label={
              labelSignature ||
              t<string>('signature {{type}}', { replace: { type: signatureType ? `(${signatureType})` : '' } })
            }
            value={signature}
            withCopy
          />
        )}
        {hash && <Static className="hash" label={labelHash || t<string>('extrinsic hash')} value={hash} withCopy />}
        {mortality && <Static className="mortality" label={t<string>('lifetime')} value={mortality} />}
        {tip?.gtn(0) && <Static className="tip" label={t<string>('tip')} value={<FormatBalance value={tip} />} />}
      </div>
    </div>
  );
}
