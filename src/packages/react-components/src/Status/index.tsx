    ...
    <div className={`item ${status}`} key={id}>
      <div className="wrapper">
        <div className="container">
          <Icon icon="times" onClick={removeItem} />
          <div className="short">
...
function renderItem({ error, extrinsic, id, removeItem, rpc, status }: QueueTx): React.ReactNode {
  ...
  return (
    <div className={`item ${status}`} key={id}>
      <div className="wrapper">
        <div className="container">
          {STATUS_COMPLETE.includes(status) && <Icon icon="times" onClick={removeItem} />}
    ...
    <div className={`ui--Status ${className}`}>
      {allSt.length + completedTx.length > 1 && (
        <div className="dismiss">
          <Button icon="times" isBasic isFull label={t<string>('Dismiss all notifications')} onClick={_onDismiss} />
        </div>
      )}
