...
function Popup({
  ...
  return (
    <div className={`ui--Popup ${className}`}>
      {isOpen && (
        <PopupWindow position={position} triggerRef={triggerRef} windowRef={dropdownRef}>
          {value}
        </PopupWindow>
      )}
      <div onClick={toggleIsOpen} ref={triggerRef}>
        {children ?? (
          <Button className={isOpen ? 'isOpen' : ''} icon="ellipsis-v" isDisabled={isDisabled} isReadOnly={false} />
        )}
      </div>
    </div>
  );
